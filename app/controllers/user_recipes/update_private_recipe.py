...
from app.schemas.user_recipes.update_private_recipe_schema import (
    UpdatePrivateRecipeSchema,
)
...
def update_private_recipe(recipe_id):
    try:
        ...
        if not private_recipe:
            raise NotFound(
                description={
                    "Error": "recipe from the id informed is not from this user or do not exist."
                }
            )
        ...
        return jsonify({"Error": error.messages}), HTTPStatus.BAD_REQUEST
